...
> An improved fork of [dsh-context-compression-selector](https://github.com/WilliamShi666/dsh-context-compression-selector) 鈥?an auditable tool-result context-compression selector for DeepSeek Harness 鈥?adding an orthogonal **code-skeleton compression gate**.

[涓枃璇存槑](README.zh.md) 路 [鏃ユ湰瑾瀅(README.ja.md) 路 [頃滉淡鞏碷(README.ko.md) 路 [Changelog](CHANGELOG.md) 路 [Installation guide](docs/installation.md)

> [!NOTE]
> **What this fork adds on top of upstream 0.1.0:**
>
> - An orthogonal **code-skeleton compression gate** (`codeSkeleton.enabled`, default off): the first exposure of an oversized fresh source-code tool result can keep a skeleton of imports and declarations 鈥?bodies elided, error lines kept 鈥?before the regular reducers run.
...
- **Code skeleton (new, orthogonal gate)** 鈥?see below.
...
Then pack the selector package and add it to a Harness profile 鈥?the full walkthrough, including verification and uninstall steps, is in the [installation guide](docs/installation.md).
...
Contributions follow the upstream discipline: add the failing regression first, keep every production change inside this repository, and explain 鈥渢riggered鈥? 鈥渆nabled but skipped鈥? and fail-open evidence separately. See [CONTRIBUTING.md](CONTRIBUTING.md).
...
- MIT 鈥?see [LICENSE](LICENSE) (upstream copyright notice retained) and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for bundled tokenizer provenance.
